(function() {
	...
	const send = text =>
		fetch(patch.join('/'), { method: 'POST', body: JSON.stringify({ text }) });
	...
	function thanks() {
		...
		header.parentNode.replaceChild(
			document.querySelector('template[name="thanks"]').content,
			header
		);
	}
})();
...
(function() {
	function init() {
		...
		['change', 'cut', 'paste', 'drop', 'keydown'].forEach(event =>
			textarea.addEventListener(event, debounced, false)
		);
	}
