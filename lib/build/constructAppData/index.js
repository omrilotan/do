...
module.exports = async function constructAppData({ activities, sources }) {
	...
	const integrations = await readFile(sources.integrations);
	appData.integrations = integrations.trim();

	// That big, round "do" button
	const playButton =  await readFile(sources.playButton);
	appData['play-button'] = playButton.trim();
